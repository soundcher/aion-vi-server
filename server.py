...
def summary():
    ...
    try:
        ...
        # no_time — надёжно, а не только по флагу с фронтенда: если часы/минуты
        # реально не переданы, время неизвестно независимо от того, прислал
        # ли конкретный вызов явный флаг no_time (не все места это делают).
        no_time = bool(data.get('no_time')) or (not data.get('hour') and not data.get('minute'))

        # Годовая профекция — тема текущего года жизни. НЕ зависит от точного
        # времени рождения (только от возраста), поэтому доступна всегда.
        ...
        # Соляр, даша, прогрессии, дирекции — все опираются на Асцендент
        # и/или точное время рождения. Без точного времени они посчитаны на
        # подставленных 12:00 и были бы не приближением, а данными другого
        # человека — поэтому при no_time просто не включаем их в промпт,
        # без каких-либо оговорок в тексте (профекции выше это не касается).
        ...
        # Маркеры совпадения — требуют минимум двух время-зависимых техник,
        # поэтому при no_time их попросту нет (профекция одна не считается).
...
def generate_analysis():
    ...
    try:
        ...
        # Этот стиль подразумевает реальную непрерывность разговора — доступен
        # ТОЛЬКО когда сессия действительно продолжается (нажали "Продолжить" и
        # conversation_history не пуста). Иначе модель имитирует несуществующую
        # непрерывность и "утаскивает" ответ в прошлую тему на пустом месте —
        # это и была причина бага с залипанием на предыдущей теме.
        if conversation_history:
            opening_styles.append("Начни без предисловий, будто продолжаешь разговор, который уже шёл.")
        chosen_opening = random.choice(opening_styles)

        # Больше НЕ упоминаем в тексте, что время рождения неизвестно —
        # вместо словесной оговорки данные, зависящие от точного времени,
        # теперь просто не попадают в промпт (см. /summary и /monthly-digest).
        no_time_note = ''
...
— ПОЛ: если в данных указан пол — согласуй с ним ВСЮ грамматику до самого конца ответа (глаголы прошедшего времени, причастия, краткие прилагательные — «ты сказал» / «ты сказала», «уставший» / «уставшая»), не только обращение в первой фразе
...
ВАЖНО: {lang_instruction}{no_time_note}{nickname_note}"""
            ...
            if is_first_generation and not conversation_history:
                intro_rule = "Это ПЕРВАЯ встреча с этим человеком. Можешь начать с короткого (2-4 предложения) живого представления — кто он по своей сути — но затем СРАЗУ переходи к его вопросу. Портрет — не самоцель, а мостик к ответу. Даже здесь избегай перечисления фактов списком (см. правило СТРУКТУРА ниже)."
            else:
                intro_rule = "Вы уже знакомы — это НЕ первая встреча. НЕ представляй его заново, НЕ описывай 'кто он есть' и 'какая у него глубина' — он это уже слышал в первой генерации. Теперь ему нужна КОНКРЕТИКА и ДЕЙСТВИЕ (см. правило КОНКРЕТИКА ниже), а не повтор портрета другими словами."
...
СТРУКТУРА — САМОЕ ВАЖНОЕ ПРАВИЛО, часто нарушается, следи за ним строго:
— ЗАПРЕЩЕНО строить ответ как список фактов о личности ("во-первых, ты... во-вторых, ты... и наконец..."), даже если списки не оформлены как список визуально, а спрятаны в прозу. Это самая частая причина, почему ответ звучит общо и "литературно" вместо конкретного.
— Тестовые пользователи прямо жалуются: ответы построены на образах (вода, антенна, поток), а не на фактаже. Это правило — прямое исправление этой жалобы.
— Вместо портрета — веди ответ от вопроса к конкретному выводу/действию/сроку. Характеристики личности используй только как аргумент по ходу дела, не как самостоятельные пункты.
— Проверь себя перед ответом: если убрать имя человека, могла бы эта фраза относиться к любому другому? Если да — перепиши конкретнее, опираясь на его данные.

КОНКРЕТИКА И ДЕЙСТВИЕ — это правило ДОБАВЛЯЕТСЯ к тёплому тону, не отменяет его:
— Пользователям не хватает именно этого: понятных дат/периодов, разрешений и запретов на решения, чёткого "иди / не иди", "бери / не бери", "решай сейчас / подожди".
— При повторных визитах (не первая генерация) — резко усиливай долю конкретики: меньше образов и метафор о характере, больше прямых указаний к действию и срокам (в рамках правила ДАТЫ ниже — только реальные даты, не выдуманные).
— Формулируй выводы как прямую рекомендацию, а не как описание: не "твоя стратегия — реагировать", а, например, "сейчас не форсируй — жди отклика" (и добавляй конкретный срок из данных, если он там есть).
— Первая генерация может оставаться тёплой и описательной (это правильно — она про знакомство). Начиная со второй генерации — фокус смещается на конкретный прогноз и действие, портретность сводится к минимуму.
...
def monthly_digest():
    ...
    try:
        ...
        # no_time — как и в /summary: надёжно, не только по флагу с фронтенда.
        no_time = bool(data.get('no_time')) or (not data.get('hour') and not data.get('minute'))

        lines = [f"АКЦЕНТ ГОДА: {prof['theme']}"]
        if dasha and not no_time:
            lines.append(f"КРУПНЫЙ ПЕРИОД: {dasha['theme']}")
        if lunar and not no_time:
        ...
        # доступна, тема месяца по лунару выше. Без точного времени даша и
        # лунар не участвуют (см. выше) — как и в /summary, без оговорок в
        # тексте, просто без этих данных в промпте.
        markers = find_markers(prof['house'], dasha['planet'] if dasha else None,
                                {'лунар': lunar['house']} if lunar else None) if not no_time else []
        if markers:
            lines.append("")
            lines.append("⚡ ПОДТВЕРЖДЁННЫЙ МАРКЕР (это НЕ догадка — минимум два независимых метода расчёта указывают ровно в одну и ту же область жизни):")
            for cat_id in markers:
